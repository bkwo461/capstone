...
const { createRDSInstance, deleteRDSInstance, infoRDSInstance, shutdownRDSInstance, poweronRDSInstance, rebootRDSInstance } = require('./rdsHelper');
const debug = false;
...
router.get('/getAllRDSInstances', async (req, res) => {
    ...
    try {
        infoRDSInstance().then(data => {
            if (data.rdsinfo.length == 0) res.status(404).json({ isError: true, message: 'No Available Instances', rawErr: data }) 
...
router.post('/getRDSInstance', async (req, res) => {
    ...
    infoRDSInstance(dbName).then(data => {
        res.status(200).json(data);
    }).catch(err => {
        if (err.message == 'No such instance found') res.status(404).json({ isError: true, message: 'No such instance found', rawErr: err });
...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        createRDSInstance(dbName, dbUsername, dbPassword).then(data => {
            res.status(200).json(data);
        }).catch(err => {
...
router.delete('/delete', async (req, res) => {
    ...
    try {
        const { dbName } = req.body;
        deleteRDSInstance(dbName).then(data => {
            res.status(200).json(data);
        }).catch(err => {
...
router.post('/shutdown', async (req, res) => {
    ...
    try {
        const { dbName } = req.body;
        shutdownRDSInstance(dbName).then(data => {
            res.status(200).json(data);
        }).catch(err => {
...
router.post('/poweron', async (req, res) => {
    ...
    try {
        const { dbName } = req.body;
        poweronRDSInstance(dbName).then(data => {
            res.status(200).json(data);
        }).catch(err => {
...
router.post('/reboot', async (req, res) => {
    ...
    try {
        const { dbName } = req.body;
        rebootRDSInstance(dbName).then(data => {
            res.status(200).json(data);
        }).catch(err => {
